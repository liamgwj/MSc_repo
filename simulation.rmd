...
Julia needs to be [installed locally](https://julialang.org/downloads/) first, then using Julia one can [install Omniscape](https://github.com/Circuitscape/Omniscape.jl#installation).

Julia also needs to be "findable" for executing, and for this, follow instructions at this [webpage](https://julialang.org/downloads/platform/).
...
```{r sim_phylo1, fig.width = 6, fig.height = 6}
...
```{r write_suitability_true1, warning=FALSE}
...
Make sure the ".ini" files use relative paths
```{r julia_connectivity_true2}
...
```{r sim_phylo2, fig.width = 6, fig.height = 6}
host_phy_lst <- sim.bd.taxa(n = 20, # number of tips on each phylogeny
...
Write suitability map to file
```{r write_suitability_true2, warning=FALSE}
for(i in 1:length(host_phy_lst)){
