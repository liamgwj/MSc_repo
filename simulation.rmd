...
This can now be used to measure "true" habitat values (connectivity etc.)
...
> I imagine this will have to be edited to return the intermediate objects as well... as it stands, works for visual comparisons
...
long_disp <- dispersal(samc_obj, occ_prob)

    # map long-term dispersal
long_disp_map <- map(samc_obj, long_disp)

return(long_disp_map)
}
```
...
```{r long_disp_calc}
long_disp_map_true <- disp_fun(max_resistance = length(phy[[1]]$tip.label) + 1,
...
Calculate connectivity
```{r long_disp_calc}
long_disp_map_known <- disp_fun(max_resistance = length(phy[[1]]$tip.label) + 1,
                                absorp_raster = absorp,
...
## Measure predicted connectivity 


Calculate connectivity
```{r long_disp_calc}
long_disp_map_pred <- disp_fun(max_resistance = length(phy[[1]]$tip.label) + 1,
                                absorp_raster = absorp,
                                host_raster = potential_host_sum)
```


Compare dispersal predictions using plots

Set common colour scale
```{r}
disp_max_value <- max(cellStats(long_disp_map_true, "max"), cellStats(long_disp_map_pred, "max"))
```


True hosts
```{r plot_true_long_dispersal}
p_disp_true <- gplot(long_disp_map_true) + 
                  geom_tile(aes(fill=value)) +
...
Known hosts
```{r plot_long_dispersal}
p_disp_known <- gplot(long_disp_map_known) + 
                  geom_tile(aes(fill=value)) +
                    scale_fill_viridis(limits = c(0, disp_max_value),
                                       option="B")
```
...
Known + predicted hosts
```{r plot_true_long_dispersal}
p_disp_pred <- gplot(long_disp_map_pred) + 
                  geom_tile(aes(fill = value)) +
                    scale_fill_viridis(limits = c(0, disp_max_value),
                                       option = "B")
